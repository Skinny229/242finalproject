...
public Grid() {
    grid = new model.Location[NUM_COLS][NUM_ROWS];
    for (int y = 0; y < NUM_ROWS; y++) {
        for (int x = 0; x < NUM_COLS; x++) {
            grid[x][y] = new model.Location();
        }
    }
}
...
// Gets the model.Location object at row and column position

public model.Location get(int row, int col){
    return grid[col][row];
}
